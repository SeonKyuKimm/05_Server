	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if(loginMember != null) {// 로그인 성공
				...
				// ( 긴 시간 로그인이 유지되면 보안상의 문제가 있을 수 있다.)
 				session.setMaxInactiveInterval(60 * 60); // 세션 생성 후 1시간으로 변경할거다
				...
				// 5 - 2 . 메인 페이지로 이동 재요청("/") (ReDirect) 
 				...
 				//			  request, response 폐기됨 -> 재요청 후 응답받은 페이지에서 사용 불가하다
 				
			}else {// 로그인 실패
					
				// 로그인 실패 메시지를 띄우려고 함.
				// request는 리다이렉트 시 폐기됨
				// -> 리다이렉트 요청 후 응답페이지에서 request 에 담긴 속성을 사용 불가,
				// -> session에 속성 세팅되면 재요청 페이지에서도 사용 가능 -!
				session.setAttribute("message", "아이디 또는 비밀번호 불일치");
				
				// 이전 페이지로 redirect( request, response 폐기됨 )
				String referer = req.getHeader("referer");
				// -> referer : 각 페이지 방문 시 남는 흔적
				// request.getHeader() : 파라미터로 referer 라는 키 전달 시, 이전 페이지 주소값을 반환해줌.
				// http://localhost:8080/
