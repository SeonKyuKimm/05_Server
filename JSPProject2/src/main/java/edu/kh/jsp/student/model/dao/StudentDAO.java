...
public class StudentDAO {
	...
	// 기본생성자로 DAO 객체가 생성될 때,
	// sql 이 작성된 xml 파일 읽어와서 참조변수 prop에 저장
	...
	public List<Student> selectAll(Connection conn) throws Exception{
		...
		try {
			...
			// 3 . Statement 객체 생성할것임 ㅎ (플레이스홀더 없어서)
			stmt = conn.createStatement();
			
			// 4 . sql 수행 후, 결과 반환받기
			rs = stmt.executeQuery(sql);
			
			// 5 . ResultSet 1행씩 접근하며 List에 옮겨닮기
